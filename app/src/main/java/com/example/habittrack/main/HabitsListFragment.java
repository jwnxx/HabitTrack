...
import com.example.habittrack.HabitAdapter;
import com.example.habittrack.Habits;
...
//            mParam2 = getArguments().getString(ARG_PARAM2);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ListView listView = root.findViewById(R.id.listViewHabits);
    ...
    public void getDataFromDatabase(ListView listView,View root){
        ...
        habitRef.child(user.getUid().toString()).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {

                for (DataSnapshot snapshot : task.getResult().getChildren()){
                    habits.add(snapshot.getValue(Habits.class));
                    habits.get(habits.size()-1).setUid(snapshot.getKey());
                }

                HabitAdapter adapter = new HabitAdapter(root.getContext(), habits);
                listView.setAdapter(adapter);
                SharedPreferences prefs=getActivity().getSharedPreferences("habits", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor=prefs.edit();
                editor.putString("habits",habits.size()+"");
                editor.commit();
